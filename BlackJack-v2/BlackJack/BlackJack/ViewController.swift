...
class ViewController: UIViewController {
    ...
    func refresh () {
        ...
        player_score.text = String(currentPlayer.score().stringScore)
        dealer_cards.text = String(game.dealer.showHand())
        player_cards.text = String(currentPlayer.showHand())
        outcome.text = String(game.checkScore(currentPlayer))
    }
    ...
    @IBAction func play(sender: UIButton) {
        ...
        currentPlayer = game.players.first!
        refresh()
        var temp:Int = moneyStart - bet_amount.text.toInt()!
        moneyStart = temp
        max_bet.text = String("$\(temp)")
        if(moneyStart - bet_amount.text.toInt()! < 0) {
